...
if user == "annabu":
    ...
    input_dir = data_dir
    log_dir = "./log"
    output_dir = "/p/tmp/annabu/projects/attrici/output"
# local:
elif user == "root":
    data_dir = "./meteo_data"
    log_dir = "./log"
...
## adapt paths when not using HPC
if user != "annabu":
    input_dir = Path(data_dir) / "input" / dataset / tile
    output_dir = Path(data_dir) / "output" / dataset / tile  # / Path.cwd().name  ## make output dir same as cwd. Helps if running more than one job.
    log_dir = Path(log_dir) / tile
...
source_file = f"rechunked_{dataset}_{variable}{hour}_{dataset}_1950_2020_t_{tile}_basd_redim_demo.nc4"
