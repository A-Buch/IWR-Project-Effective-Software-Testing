...
class estimator(object):
    ...
    def estimate_parameters(self, df, lat, lon, map_estimate, TIME0):
        ...
        outdir_for_cell = dh.make_cell_output_dir(self.output_dir, "traces", lat, lon, self.variable)
        if map_estimate:
            try:
                ...
                print(
                    "Problem with saved trace:",
                    e,
                    ". Redo parameter estimation.",
                )
                print(f"took {(datetime.now() - TIME0).total_seconds():.0f}s until find_MAP is run")
                trace, opt_result = pm.find_MAP(model=self.model, return_raw=True)
                trace["logp"] = np.array(-opt_result.fun)

                print(f"took {(datetime.now() - TIME0).total_seconds():.0f}s until find_MAP is done")
                if self.save_trace:
                    with open(outdir_for_cell, "wb") as handle:
                        free_params = {key: value for key, value in trace.items() if key.startswith("weights") or key == "logp"}
                        pickle.dump(
                            free_params,
                            handle,
    ...
    def sample(self):
        ...
        TIME1 = datetime.now()
        print("Finished job {0} in {1:.0f} seconds.".format(os.getpid(), (TIME1 - TIME0).total_seconds()))

        return trace

    def estimate_timeseries(self, df, trace, datamin, scale, map_estimate, subtrace=1000):
        # print(trace["mu"].shape, df.shape)
        trace_obs, trace_cfact = self.statmodel.resample_missing(trace, df, subtrace, self.model, self.progressbar, map_estimate)
