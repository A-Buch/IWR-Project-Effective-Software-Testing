...
def create_dataframe(nct_array, units, data_to_detrend, gmt, variable):
    ...
    ds = pd.to_datetime(nct_array, unit="D", origin=pd.Timestamp(units.lstrip("days since")))
...
def get_source_timeseries(data_dir, dataset, qualifier, variable, lat, lon):
    input_file = data_dir / dataset / pathlib.Path(variable + "_" + dataset.lower() + "_" + qualifier + ".nc4")
    ...
    tm = pd.to_datetime(nct[:], unit="D", origin=pd.Timestamp(nct.units.lstrip("days since")))
...
def get_cell_filename(outdir_for_cell, lat, lon, settings):
    return outdir_for_cell / ("ts_" + settings.dataset + "_lat" + str(lat) + "_lon" + str(lon) + settings.storage_format)
