...
class Pr(attrici.distributions.BernoulliGamma):
    ...
    def setup(self, df_subset):
        ...
        with model:
            ...
            covariates = pm.math.concatenate([xf0, tt.tile(gmt[:, None], (1, int(xf0_np.shape[1]))) * xf0], axis=1)
            covariatesv = pm.math.concatenate([xf0v, tt.tile(gmtv[:, None], (1, int(xf0_np.shape[1]))) * xf0v], axis=1)

            # pbern
            weights_pbern_longterm_intercept = pm.Normal("weights_pbern_longterm_intercept", mu=0, sigma=1)
            weights_pbern_longterm_trend = pm.Normal("weights_pbern_longterm_trend", mu=0, sigma=0.1)
            ...
            weights_pbern_fc_trend = pm.Normal("weights_pbern_fc_trend", mu=0, sigma=0.1, shape=xf0_np.shape[1])
            weights_pbern_fc = pm.math.concatenate([weights_pbern_fc_intercept, weights_pbern_fc_trend])
            logit_pbern = pm.Deterministic(
                "logit_pbern",
                tt.dot(covariates, weights_pbern_fc) + weights_pbern_longterm_intercept + weights_pbern_longterm_trend * gmt,
            )
            # pbern = pm.Deterministic("pbern", pm.math.invlogit(logit_pbern))
            # mu
            weights_mu_longterm_intercept = pm.Normal("weights_mu_longterm_intercept", mu=0, sigma=1)
            weights_mu_longterm_trend = pm.Normal("weights_mu_longterm_trend", mu=0, sigma=0.1)
            ...
            weights_mu_fc_trend = pm.Normal("weights_mu_fc_trend", mu=0, sigma=0.1, shape=xf0.shape[1])
            weights_mu_fc = pm.math.concatenate([weights_mu_fc_intercept, weights_mu_fc_trend])
            eta_mu = tt.dot(covariatesv, weights_mu_fc) + weights_mu_longterm_intercept + weights_mu_longterm_trend * gmtv
            mu = pm.Deterministic("mu", pm.math.exp(eta_mu))
            # nu
            weights_nu_longterm_intercept = pm.Normal("weights_nu_longterm_intercept", mu=0, sigma=1)
            weights_nu_fc_intercept = pm.math.concatenate(
                [pm.Normal(f"weights_nu_fc_intercept_{i}", mu=0, sigma=1 / (i + 1), shape=2) for i in range(int(xf0_np.shape[1]) // 2)]
            )
...
class Tas(attrici.distributions.Normal):
    ...
    def setup(self, df_subset):
        ...
        with model:
            ...
            weights_longterm_intercept = pm.Normal("weights_longterm_intercept", mu=0, sigma=1)
            weights_longterm_trend = pm.Normal("weights_longterm_trend", mu=0, sigma=0.1)
            ...
            weights_fc_trend = pm.Normal("weights_fc_trend", mu=0, sigma=0.1, shape=xf0_np.shape[1])
            ...
            # sigma
            weights_sigma_longterm_intercept = pm.Normal("weights_sigma_longterm_intercept", mu=0, sigma=1)
...
class Rlds(attrici.distributions.Normal):
    ...
    def setup(self, df_subset):
        ...
        with model:
            ...
            weights_fc_trend = pm.Normal("weights_fc_trend", mu=0, sigma=0.1, shape=xf0.shape[1])
            weights_fc = pm.math.concatenate([weights_fc_intercept, weights_fc_trend])
            # weights are the parameters that are learned by the model
            # eta is a linear model of the predictors
            eta = (
...
class Hurs(attrici.distributions.Beta):
    ...
    def setup(self, df_subset):
        ...
        with model:
            ...
            covariates = pm.math.concatenate([xf0, tt.tile(gmtv[:, None], (1, int(xf0_np.shape[1]))) * xf0], axis=1)

            # phi is called the precision parameter
            weights_phi_longterm_intercept = pm.Normal("weights_phi_longterm_intercept", mu=0, sigma=1)
...
class RsdsWeibull(attrici.distributions.Weibull):
    ...
    def setup(self, df_subset):
        ...
        with model:
            ...
            # alpha
            weights_alpha_longterm_intercept = pm.Normal("weights_alpha_longterm_intercept", mu=0, sigma=1)
